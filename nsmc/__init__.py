...
class GmailAccount(EmailAccount):
    def __init__(self, login):
        self.redirect_port = get_free_port()
    ...
    def get_proxy_config(self):
        return _u(f"""
            ...
            redirect_uri = http://localhost:{self.redirect_port}
...
def main():
    ...
    emails = [create_email_account(login) for login in config]
    ...
    with open(epc_path, "w", encoding="utf8") as epc_file:
        epc_file.write(create_emailproxy_config(proxies_to_ports, emails))
    ...
    for email in emails:
        ...
        print(f"{email.login} redirect proxy port {email.redirect_port}")
